...
type EmbeddedRunParams = RunEmbeddedPiAgentParams;
...
describe("runReplyAgent typing (heartbeat)", () => {
  async function withTempStateDir<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "openclaw-typing-heartbeat-"));
    try {
  ...
  it("uses configured prompts for memory flush runs", async () => {
    await withTempStore(async (storePath) => {
      ...
      registerMemoryFlushPlanResolver(() => ({
        ...
        prompt: "Write notes. NO_REPLY to memory/2023-11-14.md and MEMORY.md",
      ...
      expect(flushCall?.prompt).toContain("memory/2023-11-14.md");
