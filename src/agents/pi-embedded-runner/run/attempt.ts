...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      type LlmInputContextStage = {
        stage: string;
        messages: AgentMessage[];
        messageCount: number;
        estimatedTokens?: number;
      };
      const llmInputContextStages: LlmInputContextStage[] = [];
      const pushLlmInputContextStage = (
        stage: LlmInputContextStage["stage"],
        messages: AgentMessage[],
      ) => {
      ...
      try {
        ...
        try {
          ...
          if (hookRunner?.hasHooks("llm_input")) {
            ...
            } as Parameters<NonNullable<typeof hookRunner.runLlmInput>>[0];
            hookRunner
              .runLlmInput(llmInputEvent, {
                runId: params.runId,
                agentId: hookAgentId,
                sessionKey: params.sessionKey,
                sessionId: params.sessionId,
