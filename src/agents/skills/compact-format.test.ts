...
describe("applySkillsPromptLimits (via buildWorkspaceSkillsPrompt)", () => {
  ...
  it("budget check uses the same absolute paths that are rendered in the prompt", () => {
  ...
  it("snapshot prompt and resolvedSkills both keep canonical paths", () => {
    const home = os.homedir();
    ...
    expect(snapshot.prompt).not.toContain("~/");
